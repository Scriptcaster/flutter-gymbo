...
Future<void> createWeek(String uid) async {
  ...
  if(weeksQuerySnapshot.documents.length > 0) {
    ...
    DocumentReference monRef = await weeksCollection.document(weekRef.documentID).collection('days').add({'index': 0, 'date': 'Monday', 'target': lastExercises[0]['target'], 'cardio': lastExercises[0]['cardio'], 'exercises': lastExercises[0]['exercises']});
    weeksCollection.document(weekRef.documentID).collection('days').document(monRef.documentID).updateData({'id': monRef.documentID});

    DocumentReference tueRef = await weeksCollection.document(weekRef.documentID).collection('days').add({'index': 1, 'date': 'Tuesday', 'target': lastExercises[1]['target'], 'cardio': lastExercises[1]['cardio'], 'exercises': lastExercises[1]['exercises']});
    weeksCollection.document(weekRef.documentID).collection('days').document(tueRef.documentID).updateData({'id': tueRef.documentID});

    DocumentReference wedRef = await weeksCollection.document(weekRef.documentID).collection('days').add({'index': 2, 'date': 'Wednesday', 'target': lastExercises[2]['target'], 'cardio': lastExercises[2]['cardio'], 'exercises': lastExercises[2]['exercises']});
    weeksCollection.document(weekRef.documentID).collection('days').document(wedRef.documentID).updateData({'id': wedRef.documentID});

    DocumentReference thuRef = await weeksCollection.document(weekRef.documentID).collection('days').add({'index': 3, 'date': 'Thursday', 'target': lastExercises[3]['target'], 'cardio': lastExercises[3]['cardio'], 'exercises': lastExercises[3]['exercises']});
    weeksCollection.document(weekRef.documentID).collection('days').document(thuRef.documentID).updateData({'id': thuRef.documentID});

    DocumentReference friRef = await weeksCollection.document(weekRef.documentID).collection('days').add({'index': 4, 'date': 'Friday', 'target': lastExercises[4]['target'], 'cardio': lastExercises[4]['cardio'], 'exercises': lastExercises[4]['exercises']});
    weeksCollection.document(weekRef.documentID).collection('days').document(friRef.documentID).updateData({'id': friRef.documentID});

    DocumentReference satRef = await weeksCollection.document(weekRef.documentID).collection('days').add({'index': 5, 'date': 'Saturday', 'target': lastExercises[5]['target'], 'cardio': lastExercises[5]['cardio'], 'exercises': lastExercises[5]['exercises']});
    weeksCollection.document(weekRef.documentID).collection('days').document(satRef.documentID).updateData({'id': satRef.documentID});

    DocumentReference sunRef = await weeksCollection.document(weekRef.documentID).collection('days').add({'index': 6, 'date': 'Sunday', 'target': lastExercises[6]['target'], 'cardio': lastExercises[6]['cardio'], 'exercises': lastExercises[6]['exercises']});
